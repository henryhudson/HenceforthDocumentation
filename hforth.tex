...
\begin{tikzpicture}[remember picture, overlay]
  ...
  \fill[forthBlue, opacity=0.15] ($(current page.north east)+(-4,-3)$) circle (6cm);
  \fill[forthCyan, opacity=0.10] ($(current page.south west)+(3,4)$) circle (5cm);
  \fill[forthGold, opacity=0.08] ($(current page.south east)+(-2,2)$) circle (3cm);
  \fill[forthBlue] ($(current page.west)+(2,1)$) rectangle ($(current page.west)+(14,1.08)$);
  \node[anchor=west, text=white, font=\fontsize{52}{56}\selectfont\bfseries\sffamily]
    at ($(current page.west)+(2,3.5)$) {Henceforth};
  \node[anchor=west, text=forthCyan, font=\Large\sffamily]
    at ($(current page.west)+(2,1.8)$) {A FORTH Programming Environment for iOS};
  \node[anchor=west, text=forthGrey, font=\large\sffamily]
    at ($(current page.west)+(2,-0.5)$) {Henry Hudson};
  \node[anchor=south east, text=forthGrey!60, font=\small\sffamily]
    at ($(current page.south east)+(-2,1.5)$) {Documentation \(\cdot\) 2026};
  \foreach \y/\val/\col in {-3/42/forthBlue, -4/17/forthCyan, -5/8/forthGold, -6/3/forthOrange} {
    \fill[\col, opacity=0.7, rounded corners=2pt]
      ($(current page.east)+(-5.5,\y)$) rectangle ($(current page.east)+(-2.5,\y+0.7)$);
    \node[text=white, font=\ttfamily\bfseries] at ($(current page.east)+(-4,\y+0.35)$) {\val};
  }
  \node[text=forthGrey!50, font=\sffamily\small] at ($(current page.east)+(-4,-7)$) {LIFO Stack};
\end{tikzpicture}
\end{titlepage}
...
\begin{forthbox}[Terminal Operators]
  ...
  (            (--)
               Starts comment. Everything between braces will be
               ignored.
  ...
  key          ( -- n)
               Adds the ASCII code to the stack from the ASCII
               character provided.

  key?         ( -- flag)
               Returns true if a key has been pressed, false
               otherwise. Non-blocking check for keyboard input.
...
\item \textbf{Fee Calculation:} Fees are calculated using the current rate from the selected miner (GorillaPool or TAAL) via the Merchant API (mAPI).
...
\item \textbf{Broadcasting:} The signed transaction is submitted via ARC (next-gen) or mAPI with automatic failover between GorillaPool and TAAL.
\end{enumerate}
...
\begin{notebox}
\textbf{Balance Reconciliation:} The displayed balance is always derived from the UTXO set, never from a separate API call. This guarantees that what you see matches what you can spend:
\begin{lstlisting}
