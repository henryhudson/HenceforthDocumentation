...
Watch-only address monitoring            & \texttt{MempoolMonitorService} + \texttt{HFAddressPack} push        & $\circ$\,\textsuperscript{e} \\
...
\textsuperscript{e} Export side complete: signer derives a list of receive addresses and writes a signed \texttt{.hfaddrpack} bundle that the user AirDrops to the watcher. Watcher-side import is partial in v1.0 --- the bundle decodes and validates, but the watcher does not yet persist the addresses into a watch-only store wired to \texttt{MempoolMonitorService}. Until that wiring lands, the watcher receives the addresses but cannot act on them; v1.1 closes the loop.
\end{minipage}
\end{center}

Offline header transfer and Merkle-proof export are now closed end-to-end. \texttt{HFHeaderBundle} carries a contiguous run of headers plus the source checkpoint hash; the receiving signer imports them through the existing \texttt{commitValidatedHeaderRun} path so chain-link + PoW + checkpoint-anchor invariants enforce themselves on import. \texttt{HFReceiptBundle} carries a raw transaction + TSC (BRC-74) Merkle proof + the containing block header; \texttt{verifyTransaction(fromBundle:)} validates inclusion against the signer's local header at the same height without any network call. The watch-only address-pack flow is asymmetric in v1.0: \texttt{HFAddressPack} encodes and signs the address list on the signer (where the seed lives) and the watcher decodes and validates the signature, but persistence into a monitoring store is staged for v1.1 --- the format is final, the import path is honest about what it does today. The Settings Air-Gap Tools section presents all three via system file pickers, with AirDrop reachable from the iOS and macOS share sheets.
